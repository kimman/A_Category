...
#define YIPMP4MuxerAddOutputError 1000
#define YIPMP4MuxerMaxQueueCount 10000

// 封装器的状态机。
typedef NS_ENUM(NSInteger, YIPMP4MuxerStatus) {
    YIPMP4MuxerStatusUnknown = 0,
    YIPMP4MuxerStatusRunning = 1,
    YIPMP4MuxerStatusFailed = 2,
    YIPMP4MuxerStatusCompleted = 3,
    YIPMP4MuxerStatusCancelled = 4,
};

@interface YIPMP4Muxer ()
{
    CMSimpleQueueRef _audioQueue; // 音频数据队列。
    CMSimpleQueueRef _videoQueue; // 视频数据队列。
}

@property (nonatomic, strong, readwrite) YIPMuxerConfig *config;
...
@property (nonatomic, assign) YIPMP4MuxerStatus muxerStatus;

@end

@implementation YIPMP4Muxer
...
- (instancetype)initWithConfig:(YIPMuxerConfig *)config {
    ...
    if (self) {
        ...
        CMSimpleQueueCreate(kCFAllocatorDefault, YIPMP4MuxerMaxQueueCount, &_audioQueue);
        CMSimpleQueueCreate(kCFAllocatorDefault, YIPMP4MuxerMaxQueueCount, &_videoQueue);
    }
...
- (void)startWriting {
    ...
    dispatch_async(self.muxerQueue, ^{
        ...
        [weakSelf _reset]; // 清理。
        weakSelf.muxerStatus = YIPMP4MuxerStatusRunning; // 标记状态。
...
- (void)cancelWriting {
    ...
    dispatch_async(self.muxerQueue, ^{
        ...
        weakSelf.muxerStatus = YIPMP4MuxerStatusCancelled; // 标记状态。
        dispatch_semaphore_signal(weakSelf.semaphore);
    });
}

- (void)appendSampleBuffer:(CMSampleBufferRef)sampleBuffer {
    if (!sampleBuffer || !CMSampleBufferGetDataBuffer(sampleBuffer) || self.muxerStatus != YIPMP4MuxerStatusRunning) {
        return;
    }
    ...
    dispatch_async(self.muxerQueue, ^{
        ...
        if (!weakSelf.muxWriter) {
            ...
            if (![weakSelf _checYIPormatDescriptionLoadSuccess]) {
            ...
            if (!success) {
                weakSelf.muxerStatus = YIPMP4MuxerStatusFailed;
                CFRelease(sampleBuffer);
                dispatch_semaphore_signal(weakSelf.semaphore);
...
- (void)stopWriting:(void (^)(BOOL success, NSError *error))completeHandler {
    ...
    dispatch_async(self.muxerQueue, ^{
        ...
        [weakSelf _stopWriting:^(BOOL success, NSError *error) {
            weakSelf.muxerStatus = success ? YIPMP4MuxerStatusCompleted : YIPMP4MuxerStatusFailed;
            dispatch_semaphore_signal(weakSelf.semaphore);
            if (completeHandler) {
...
- (BOOL)_setupMuxWriter:(NSError **)error {
    if (!self.config.outputURL) {
        *error = [NSError errorWithDomain:NSStringFromClass([YIPMP4Muxer class]) code:40003 userInfo:nil];
        return NO;
    }
    ...
    if ((self.config.muxerType & YIPMediaVideo) && !_writerVideoInput) {
        ...
        if ([self.muxWriter canAddInput:self.writerVideoInput]) {
            ...
            *error = self.muxWriter.error ? self.muxWriter.error : [NSError errorWithDomain:NSStringFromClass([YIPMP4Muxer class]) code:YIPMP4MuxerAddOutputError userInfo:nil];
            return NO;
        }
    }
    
    // 4、当封装内容包含音频时，创建 Muxer 的音频输入。
    if ((self.config.muxerType & YIPMediaAudio) && !_writerAudioInput) {
...
- (void)_avInterLeavedSample {
    ...
    if ((self.config.muxerType & YIPMediaAudio) && (self.config.muxerType & YIPMediaVideo)) { // 同时封装音频和视频。
    ...
    } else if (self.config.muxerType & YIPMediaAudio) { // 只封装音频。
        [self _appendAudioSample];
    } else if (self.config.muxerType & YIPMediaVideo) { // 只封装视频。
        [self _appendVideoSample];
    }
}

- (BOOL)_checYIPormatDescriptionLoadSuccess {
    // 检查数据是否正常。
    if (!_muxWriter) {
        if ((self.config.muxerType & YIPMediaAudio) && (self.config.muxerType & YIPMediaVideo)) {
            return CMSimpleQueueGetCount(_videoQueue) > 0 && CMSimpleQueueGetCount(_audioQueue) > 0;
        } else if (self.config.muxerType & YIPMediaAudio) {
            return CMSimpleQueueGetCount(_audioQueue) > 0;
        } else if (self.config.muxerType & YIPMediaVideo) {
