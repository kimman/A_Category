...
@property (nonatomic, strong, readwrite) YIPAudioConfig *config;
...
@implementation YIPAudioCapture
...
- (instancetype)initWithConfig:(YIPAudioConfig *)config {
    ...
    if (self) {
        _config         = config;
        _captureQueue   = dispatch_queue_create("com.KimmsObjCDemo.audioCapture", DISPATCH_QUEUE_SERIAL);
    }
    return self;
}
...
static OSStatus audioBufferCallBack(void *inRefCon,
    ...
    @autoreleasepool {
        YIPAudioCapture *capture = (__bridge YIPAudioCapture *) inRefCon;
        if (!capture) {
            return -1;
        }
        ...
        if (status == noErr) {
            ...
            CMSampleBufferRef sampleBuffer = [YIPAudioCapture sampleBufferFromAudioBufferList:buffers inTimeStamp:inTimeStamp inNumberFrames:inNumberFrames description:capture.audioFormat];
            // 回调数据。
